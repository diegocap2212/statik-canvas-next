...
Observações IA por etapa: ${JSON.stringify(session.aiCache, null, 2)}`;
  ...
  const rawResult = await callClaude(DIAGNOSIS_PROMPT, summary);
  ...
  try {
    ...
    const fixed = await callClaude("Retorne apenas JSON válido, sem markdown.", fixPrompt);
